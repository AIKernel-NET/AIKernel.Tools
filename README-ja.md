...
AIOS SDK において、AIKernel.Tools は observability / instrumentation layer です。
CLI operation、replay、inspector、canonical formatting、operator workflow を提供し、
diagnostics が不要な host からは安全に外せる補助層として AIOS distribution に並走します。
...
governance、observability を統合する標準 AIOS として開発が開始されています。
